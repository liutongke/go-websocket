...
func getGormConfig() *gorm.Config {
	return &gorm.Config{
		NamingStrategy: schema.NamingStrategy{
			...
			NoLowerCase: true, //是否禁用表名和字段名的小写转换，如果设置为 true，则表名和字段名将保持原样，不进行小写转换。举个例子，如果数据库中的表名是 "UserInfo"，默认情况下 GORM 会将其转换为小写形式 "userinfo"。但是如果将 NoLowerCase 设置为 true，那么 GORM 将保持表名的原始大小写，即 "UserInfo"。同样的规则也适用于字段名。
		},
		Logger:                                   getLogger(),
