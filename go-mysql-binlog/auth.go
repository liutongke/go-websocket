...
func main() {
	...
	pwd := hex.EncodeToString(encodePass("root"))
	sprintf := fmt.Sprintf("%s%s%s", str1, userName, pwd)
	fmt.Println(sprintf, len(sprintf))
	data := "85a6030000000040210000000000000000000000000000000000000000000000726f6f7400144b9d5c36cafbf59426ffd1180364a69927539695"
...
func encodePass(pwd string) []byte {
	//bytes, _ := hex.DecodeString("4b9d5c36cafbf59426ffd1180364a69927539695")
	//fmt.Println(bytes)
	//两个盐值相加
	decodeString1, _ := hex.DecodeString("4b79225e1f0a2915")
	...
	arr3 := append(decodeString1, decodeString2...)
	pass := scramblePassword(arr3, pwd)
